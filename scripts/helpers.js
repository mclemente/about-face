...
export function getRotationDegrees(dX = null, dY = null, dir = null, isHexColumn = false) {
	...
	if ((dX == 0 && dY < 0) || dir == "up") rotation = 180;
	// up
	else if ((dX == 0 && dY > 0) || dir == "down") rotation = 0;
	// down
	else if ((dX > 0 && dY == 0) || dir == "right") rotation = 270 - hexOffset * 2;
	// to the right (hex columns can't go right/left)
	else if ((dX > 0 && dY < 0) || dir == "up-right") rotation = 225 + hexOffset;
	// up to the right
	else if ((dX > 0 && dY > 0) || dir == "down-right") rotation = 315 - hexOffset;
	// down to the right
	else if ((dX < 0 && dY == 0) || dir == "left") rotation = 90 - hexOffset * 2;
	// to the left
	else if ((dX < 0 && dY > 0) || dir == "down-left") rotation = 45 + hexOffset;
	// down to the left
