...
-   You can lock tokens by either double-clicking and selecting the token icon from the hud or accessing from the character sheet. The option you're looking for is "Lock Rotation" under the "Position" tab. If you want this on a global level, toggle the `Lock Rotation` module setting on and every token's rotation will be locked by default.
